...
zip_app() {
  ...
  if ! codesign --verify --deep --strict "$t/$(basename "$1")" 2>/dev/null; then
    echo "✗ Signature invalid after unzipping; this package must not ship. Checking the zip for ._* leftovers:" >&2
...
if [[ "$MODE" == "release" || "$MODE" == "share" ]]; then
  echo "Compiling a universal binary (arm64 + x86_64, minimum macOS $MIN_MACOS)..."
  ...
  echo "Compiling (${#SRC[@]} source files, host architecture only)..."
  swiftc -O -swift-version 5 -target arm64-apple-macos$MIN_MACOS \
...
# Icon (generated by icon/make-icons.sh — if you change the shape of the sun, run it again)
...
if [[ "$MODE" == "release" ]]; then
  ...
  if [[ -z "$ID" ]]; then
    ...
    echo "  Download one under Xcode › Settings › Accounts, or create it at developer.apple.com."
    exit 1
  fi
  echo "Signing as: $ID"
  ...
  zip_app "$APP" "$ZIP"
  echo "Submitting for notarisation (anywhere from seconds to minutes)..."
...
if [[ "$MODE" == "share" ]]; then
  ...
  rm -f "$OUT"
  zip_app "$APP" "$OUT"
  echo "✓ Distributable: $OUT (quarantine must be cleared on first open — see dist/INSTALL.txt)"
fi
